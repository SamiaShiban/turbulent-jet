...
Ealpha(k+1)=alpha*rhoA*2*pi*b(k+1)*h(k+1)*U(k+1);
 ...
 if ws00>alpha*cos(phi(k))*U(k)
    ...
    ru=(ws00-alpha*U(k)*cos(phi(k)))*(2*b(k)-D)/Q(k)*(1-alpha*U(k)*cos(phi(k))/ws00)*alpha*b(k)^2/D^2*deltaS(k);
        bu=b(k);
end
...
steps=k
Distance=DELTAS(k)
xe=[0.1 0.2 0.32 0.5 0.55 0.65 0.75 0.82 0.92 1.2 1.4 1.9 3.5];
Se=[0.13 0.42 0.56 0.7 0.65 0.75 0.55 0.58 0.56 0.5 0.38 0.3 0.05];
...
ylabel('Dépôt (gr/m.s)')
title ('Cas 4')
legend('Modèle proposé',' Données expérimentales (cas 4) [Lane-Serff et Moran 2005]',1)
