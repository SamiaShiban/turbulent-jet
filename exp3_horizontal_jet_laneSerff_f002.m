...
Ealpha(k+1)=alpha*rhoA*2*pi*b(k+1)*h(k+1)*U(k+1);
 ...
 if ws00>alpha*cos(phi(k))*U(k);
  Pl(k+1)=Pl(k)-1*(ws00-alpha*U(k)*cos(phi(k)))*(2*b(k))/Q(k)*(1-alpha*U(k)*cos(phi(k))/ws00)*deltaS(k);
    ru=(ws00-alpha*U(k)*cos(phi(k)))*(2*b(k)-D)/Q(k)*deltaS(k);
    r(k+1)=1-alpha*U(k+1)*cos(phi(k+1))/ws00;
        bu=b(k);
end
...
DELTAD(k+1)=DELTAS(k+1)/D;
time(k+1)=time(k)+deltat;
if abs(alpha*cos(phi(k))*U(k)-ws00)<0.0001
dist=deltaS(k)
entvel=alpha*U(k)
slice=k
axvelo=U(k)
...
steps=k
Distance=DELTAS(k)
xe=[0.17 0.4 0.7 0.85 1.125 1.5 1.75 2.15 2.75 3.9];
Se=[0.12 0.42 0.55 0.58 0.5 0.45 0.3 0.22 0.1 0];
...
axis([-0.05 0.4 0 1])
title ('Cas 3')
legend('Modèle proposé',' Données expérimentales (cas 3) [Lane-Serff et Moran 2005]',1)
...
figure(5)
[AX,H1,H2] = plotyy(DELTAD,U,DELTAD,r);
hold on
xlabel('Axe du jet  ({\it S/D})')
plot(DELTAD,u,'c')
plot(DELTAD,w,'g')
set(get(AX(1),'Ylabel'),'String','Vitesse du jet (m/s)') 
set(get(AX(2),'Ylabel'),'String','1-\alpha.V.cos\phi / w_s ') 
plot(DELTAD,u)
plot(DELTAD,w)
legend('Vitesse axiale du jet ({\it V}  )',' Vitesse horizontale du jet ({\it u} )', ' Vitesse verticale du jet ({\it w} )',1)
hold off
